...
public class PalantiriPresenter {
    ...
    private void beginBeingThreads(int beingCount) {
        ...
        //mBeingThreads = Stream.of(beingCount).map(i -> new Thread(new BeingRunnable(this))).collect(Collectors.toList());

        mBeingThreads = Stream.generate(()->new Thread(new BeingRunnable(this))).limit(beingCount).collect(Collectors.toList());
