...
public class PalantiriPresenter {
    ...
    private void beginBeingTasksGazing(int beingCount) {
        ...
        mBeingsAsyncTasks = new ArrayList<>(beingCount);

        Stream.generate(() -> new BeingAsyncTask()).limit(beingCount).forEach(t -> mBeingsAsyncTasks.add(t));

        LinkedBlockingQueue queue = new LinkedBlockingQueue();

        mThreadPoolExecutor = new ThreadPoolExecutor(beingCount, beingCount, 5000, TimeUnit.MILLISECONDS, queue, mThreadFactory);

        mBeingsAsyncTasks.forEach(t -> mThreadPoolExecutor.execute(() -> t.doInBackground(this)));
    }
    ...
    private void awaitTerminationOfThreadPoolExecutor() {
        ...
        new Thread(() -> {
            try {
                ...
                // TODO -- you fill in here.
                mThreadPoolExecutor.awaitTermination( 5000, TimeUnit.MILLISECONDS);
    ...
    public void shutdown(boolean abnormalShutdown) {
        synchronized(this) {
            ...
            // TODO - you fill in here.
            mBeingsAsyncTasks.forEach(t -> t.cancel(true));
